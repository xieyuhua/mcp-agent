...
type Config struct {
	...
	// SkillsDir 技能目录：存放 .md 技能文件，Agent 启动时扫描加载，
	// 由大模型通过 use_skill 工具按需加载执行。留空默认 "skills"。
	SkillsDir string `json:"skills_dir"`
}

// RAGConfig 知识库检索增强生成配置。
type RAGConfig struct {
	Enabled       bool   `json:"enabled"`
	Source        string `json:"source"`        // 知识库来源：本地文件路径、API 地址
	EmbeddingModel string `json:"embedding_model"` // 向量模型名称
	TopK          int    `json:"top_k"`          // 检索返回 top-k 条
...
type AgentConfig struct {
	// Mode 运行模式：react（默认，ReAct 循环）| plan（先生成计划再逐步执行）。
	Mode string `json:"mode"`
	...
	// MaxResultChars 工具返回结果（非 JSON 行数截断场景）字符截断上限，防止上下文爆炸。
	// 0 表示不截断（默认）；>0 时超过该字符数将被截断并提示。
...
重要约束：
- 你自身没有写死的内置技能，所有通用能力都来自后端 MCP 服务提供的工具（数据库查询、文件读写、天气、图表等），请以工具列表中的实际定义为准；
- 当用户的请求匹配某类标准化流程（如数据质量检查、周期报表生成、异常排查等）时，可调用 use_skill 工具加载对应的预定义技能（工作流指引），并严格按其正文步骤执行；技能由 skills 目录提供，无需你自行编造流程；
...
- 权限隔离、数据脱敏、危险 SQL 拦截由 Agent 自身管理（基于配置的数据库角色权限策略），文件路径越界拦截由后端 MCP 服务处理，你只需专注生成正确的分析与操作；
...
- 【需求不明确时先澄清】：如果用户的问题过于笼统或缺失关键条件（如时间范围、产品类别、分析维度等），不要盲目猜测或直接给出可能不相关的结果。请先列出 2~3 个关键澄清问题让用户选择确认，用户回答后再继续分析。例如用户说"销售数据"时，可问："1) 想看哪个时间段的？本月/本季度/今年？2) 按产品线还是按区域分析？3) 关注哪个指标：销售额、订单量还是利润？"。`
...
当用户的请求匹配某类标准化流程时，可调用 use_skill 工具加载对应的预定义技能（工作流指引）并严格按其执行；技能由 skills 目录提供。
...
- 【需求不明确时先澄清】：如果用户的问题过于笼统或缺失关键条件（如时间范围、分析维度等），先列出 2~3 个关键澄清问题让用户选择确认，不要盲目猜测。`
