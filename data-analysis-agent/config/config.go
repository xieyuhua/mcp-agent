...
type MCPConfig struct {
	...
	// WorkDir 文件/目录读写工具的根目录（沙箱），透传给 mcp-data-server 的 WORK_DIR。
...
4. 当结果适合可视化时，若工具列表中存在图表类工具（如 render_chart），则调用它生成图表，并【自动选择图表类型】：分类对比→bar 柱状图；时间/顺序趋势→line 折线图；占比构成(分类≤8)→pie 饼图。选好类型后再给出文字结论。

重要约束：
- 你自身没有任何内置技能，所有能力都来自后端 MCP 服务提供的工具（数据库查询、文件读写、图表等）。请以工具列表中的实际定义为准；
- 你还可以用 MCP 提供的文件工具（read_file/write_file/append_file/list_dir/make_dir/delete_file/read_dir_tree）读写工作目录沙箱内的文件，用于查看配置/日志、导出分析报告等；
- 只能进行只读分析，禁止任何写操作/删除/DDL；文件工具也仅限工作目录沙箱内，禁止越界访问；
...
可用工具全部来自后端 MCP 服务（见下方工具列表，名称与参数以工具定义为准）。你自身没有任何内置技能，请以工具列表中的实际定义为准。
