...
修改或新增技能文件后，有以下方式让 Agent 重新加载：

1. **后台管理页面**：进入「技能管理」页面，点击「一键热重载」按钮（需 `skill:write` 权限）。
2. **API 直接调用**：`POST /api/admin/reload-skills`（需 `skill:write` 权限）。
3. **配置变更触发**：在后台修改 `skills_dir` 配置并保存，`ApplyConfig` 自动检测变化并重新加载技能。

## 新增一个技能

1. 在 `skills/` 下新建 `<your_skill>.md`；
2. 写好 `name` / `description` / 正文工作流；
3. 在后台「技能管理」页面点击「一键热重载」即可生效（无需重启进程）。
