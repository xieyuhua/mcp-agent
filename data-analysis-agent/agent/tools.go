...
type toolRunner func(args map[string]interface{}, result *AskResult, onProgress func(message string)) (string, error)
...
func (a *Agent) buildRegistry(specs []llm.Tool) map[string]toolRunner {
	...
	for _, s := range specs {
		...
		if cli, ok := a.toolRoute[name]; ok {
			reg[name] = func(args map[string]interface{}, result *AskResult, onProgress func(message string)) (string, error) {
				return a.callExtraMCP(cli, name, args, result, onProgress), nil
			}
			continue
		}
		// 其余统一走主 MCP（内置 mcp-data-server 或通用远程 MCP）。
		reg[name] = func(args map[string]interface{}, result *AskResult, onProgress func(message string)) (string, error) {
			return a.callMainMCP(name, args, result, onProgress), nil
		}
	}
	return reg
}
...
func (a *Agent) callMainMCP(name string, args map[string]interface{}, result *AskResult, onProgress func(message string)) string {
	mcpName := name
	if a.builtin {
		args["token"] = a.token
...
func (a *Agent) callExtraMCP(cli *mcpclient.Client, name string, args map[string]interface{}, result *AskResult, onProgress func(message string)) string {
