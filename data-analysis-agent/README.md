...
- **工具并发执行（可配）**：同一轮 LLM 返回多个工具调用时，可通过 `agent.tool_concurrency`（默认 `1` = 串行）开启并发，所有工具并行执行、全部完成后统一回灌模型；前端按工具 ID 归类到独立卡片，并发输出不混乱（详见设计要点）。
...
```json
{
  ...
  "agent": {
    ...
    "tool_concurrency": 1                 // 同轮多个工具调用并发上限：<=1 串行（默认），>1 并发执行后统一回灌
  },
  "log": {
...
| | `tool_concurrency` | `1` | 同轮多个工具调用并发上限（<=1 串行，>1 并发） |
...
1. **后台一键重载**（推荐）：进入后台「技能管理」页面，点击「一键热重载」按钮（需 `skill:write` 权限）。页面会展示当前已加载技能列表，重载后即时刷新。
   ```powershell
...
- **工具并发执行（无数据竞争、前端不乱）**：当 `agent.tool_concurrency > 1` 时，同一轮 LLM 返回的多个 `tool_calls` 在各自 goroutine 中并发执行，每个工具写入**独立的 shadow `AskResult`**，彻底避免对共享 `result` 的并发写竞争；所有进度/结果流式事件都携带 `tool_call_id`（即 `StepLog.ID`），经同一个 channel 汇总到主协程**单点写出 SSE**；主协程 `WaitGroup` 等待全部完成后，按原始顺序合并 shadow 到主 `result` 并回灌 messages（每个 `tool` 消息与 `tool_call_id` 一一对应）。前端按 `id` 把每个工具归类到独立「分析过程」卡片，并发期间多张卡片各自实时更新，互不覆盖。设 `<=1` 则退回原有串行逻辑（零风险）。
