...
- 暴露给大模型的工具**直接取自远程服务的 `tools/list` 真实清单**（外加 Agent 内置的 `render_chart` 图表工具），因此可对接任意能力的 MCP 服务。
...
- **分析过程完整不截断**：展开的「分析过程」面板完整展示每一步的工具名 / 参数 / 返回结果（后端不再按字符截断），超长结果默认限高并支持「展开全部 ▾ / 收起 ▴」；历史会话回放同样完整可展开。
...
- **结果处理**：喂给大模型的工具返回最多保留 `max_result_rows` 行（防止上下文膨胀）；但**展示层不截断**——聊天页「分析过程」完整呈现工具返回，超长内容可展开查看。
