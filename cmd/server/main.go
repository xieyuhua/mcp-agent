...
func main() {
	...
	querySvc := service.NewQueryService(queryRepo, auditSvc, permSvc)
	webSvc := service.NewWebService(cfg.SearchProvider)
	weatherSvc := service.NewWeatherService()

	authSvc := service.NewAuthService(permRepo, cfg.SessionSecret)

	// 首次安装：无任何后台账号时自动创建 admin 并打印初始账号密码
	if username, password, created, err := authSvc.EnsureBootstrapUser(); err != nil {
		log.Fatalf("bootstrap admin user: %v", err)
	} else if created {
		log.Print(service.FormatBootstrapBanner(username, password))
	}

	adminHandler := handler.NewAdminHandler(permRepo, permSvc, authSvc, cfg.DBDialect, cfg.DBDSN, llm.NewClient(cfg.LLM))
	...
	case "http":
		if err := startHTTP(ctx, cfg, mcpServer, toolHandler, adminHandler); err != nil && ctx.Err() == nil {
...
func startHTTP(ctx context.Context, cfg *config.Config, mcpServer *mcpserver.MCPServer, toolHandler *handler.ToolHandler, adminHandler *handler.AdminHandler) error {
	addr := cfg.HTTPAddr
	if addr == "" {
		addr = ":8081"
	}
